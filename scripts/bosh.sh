...
127.0.0.1 bosh.sslip.io
EOF'

mkdir ~/certs
cat >> ~/certs/ca.pem <<EOF
-----BEGIN CERTIFICATE-----
...
cat >> ~/.profile <<EOF
export BOSH_CA_CERT=~/certs/ca.pem
export BOSH_ENVIRONMENT=bosh.sslip.io
EOF
source ~/.profile

while [[ $(curl -k -s -I -o /dev/null -w %{http_code} https://bosh.sslip.io:25555/info) != 200 ]]; do
  sleep 1
done
